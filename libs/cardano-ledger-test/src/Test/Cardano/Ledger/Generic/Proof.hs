...
specialize ::
  ...
  ( constraint (ShelleyEra (Crypto era)),
    constraint (AllegraEra (Crypto era)),
    constraint (MaryEra (Crypto era)),
    constraint (AlonzoEra (Crypto era)),
    constraint (BabbageEra (Crypto era)),
    constraint (ConwayEra (Crypto era))
  ) =>
  Proof era ->
  (constraint era => t) ->
  t

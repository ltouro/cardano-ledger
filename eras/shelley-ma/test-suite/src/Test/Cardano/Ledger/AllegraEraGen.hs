...
import Cardano.Ledger.Shelley.Tx (pattern ShelleyTx, pattern ShelleyEraTxWits)
...
instance (CryptoClass.Crypto c, Mock c) => EraGen (AllegraEra c) where
  ...
  genEraWitnesses _scriptinfo setWitVKey mapScriptWit = ShelleyEraTxWits setWitVKey mapScriptWit mempty
